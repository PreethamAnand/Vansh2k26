...
export async function POST(req: Request) {
    try {
        ...
        let captainEmail = registration.email || "";
        ...
        if (captainEmail && process.env.RESEND_API_KEY) {
            try {
                ...
                // Generate composite image (Template is 2000x647)
                const ticketBuffer = await sharp(templateBuffer)
                    .composite([
                        {
                            input: qrBuffer,
                            top: 60,
                            left: 1575 // Centered in the yellow area (x ~1540 to ~1950)
                        },
                        {
                            input: svgText,
                            top: 420,
